...
export function useUrlNavigation() {
  ...
  const updateUrlWithBack = useCallback((updates: Partial<UrlState>, replace = false) => {
  ...
  const navigateToPhase = useCallback((phase: Phase, additionalParams: Partial<UrlState> = {}) => {
    updateUrlWithBack({ 
      phase, 
  ...
  const navigateToStep = useCallback((step: Step, additionalParams: Partial<UrlState> = {}) => {
  ...
  const setBaseNumber = useCallback((baseNumber: number) => {
    updateUrlWithBack({ baseNumber, step: 'range' });
  }, [updateUrlWithBack]);

  const setRange = useCallback((rangeMin: number, rangeMax: number) => {
    updateUrl({ rangeMin, rangeMax });
  }, [updateUrl]);

  const setIsSquareNumbers = useCallback((isSquareNumbers: boolean) => {
  ...
  const getBackUrl = useCallback(() => {
    ...
    return currentState.backUrl ? `?${decodeURIComponent(currentState.backUrl)}` : '/?phase=learning';
  }, [getCurrentState]);
